...
export class Page extends BaseComponent<IPageDataInterface> {
	...
	constructor(container: HTMLElement, protected events: IEventsInterface) {
		...
		this._counter = getElementFromUtils<HTMLElement>(
			'.header__basket-counter',
			container
		) as HTMLElement | null;
		this._catalog = getElementFromUtils<HTMLElement>(
			'.gallery',
			container
		) as HTMLElement | null;
		this._wrapper = getElementFromUtils<HTMLElement>(
			'.page__wrapper',
			container
		) as HTMLElement | null;
		this._basket = getElementFromUtils<HTMLElement>(
			'.header__basket',
			container
		) as HTMLElement | null;
